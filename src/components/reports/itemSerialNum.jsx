...
export default function ItemSerialNum({ data, setData }) {
  const [arg, setArg] = useState();
  const columns = [
    { key: "index", label: "Invoice Date",type:"string" },
    { key: "Name", label: "Item Name",type:"string" },
    { key: "salesPrice", label: "Sale Price",type:"number"  },
    { key: "purchasePrice", label: "Purchase Price",type:"number"  },
    { key: "stock", label: "Total stock" ,type:"number" },
    { key: "stockWorth", label: "Stock Value",type:"number"  },
  ];
  const sendingArray = data?.items
    //   ?.filter((element, index) => (arg ? element?.Name?.includes(arg) : true))
    ?.filter((element, index) => false)
    .map((ele, index) => {
      return {
        ...ele,
        index: index + 1,
        ...
        stockWorth: ele.stock ? ele.stock * ele.salePrice : "no opening stock",
