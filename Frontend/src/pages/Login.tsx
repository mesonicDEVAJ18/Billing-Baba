...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (authMethod === 'otp') {
      ...
      setIsLoading(true);
      try {
        const res = await verifyOtp(phone, code, otpId);
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-600 to-red-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md animate-fadeIn">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-red-600">Vyapar</h1>
        ...
        {authMethod === 'otp' && showOtpInput && (
          <form onSubmit={handleLogin}>
            ...
            <div className="flex gap-2 mb-2">
              {otp.map((digit, idx) => (
                <input key={idx} ref={otpInputRefs[idx]} maxLength={1} value={digit} onChange={(e) => handleOtpChange(idx, e.target.value)}
            ...
            <button type="button" onClick={handleResendOtp} className="text-red-500 text-sm mb-4">Resend OTP</button>
